...
## H Cover Flow（11 條，對 spec＝Plan §4.8；其中 H-01／H-11 已隨 Cover Flow × 找歌詞 分支作廢，見下方 🚫）
...
| H-02 | 3D 形態五要素：中心正面／兩側 ±55° 斜角／負間距覆疊／倒影漸變／viewAligned 吸附 | Plan §4.8 | **幾何參數**已由 `CoverFlowGeometryTests`（15 條）釘住：旋轉 ±55° 含 clamp、縮放 1.0→0.82、負間距 −0.42×itemWidth、zIndex −\|d\|、perspective 0.55、首尾可達中心的 edgePadding。**觀感**（實際吸附行為、3D 疊放、resize）待 M8 並排目視——同 A-01／B-20 先例，依 Plan §1「視覺盡可能貼近（非像素級）」不做截圖比對。**U7 裁決（2026-09-23，`docs/plans/2026-09-23-coverflow-queue-drawer.md` §11）**：本分支上 `CoverFlowUITests`（原 H-02 閘門）退出回歸集合——資料模型已由整張專輯變為歷史／佇列，20 首 fixture 與探針前提不成立；檔案保留（Python 閘門測試 `Scripts/test_h02_gate_parsing.py` 仍讀它）但不再作本條證據。替代證據：`Tests/UI/CoverFlowStripStackingTests.swift`（缺陷 2 疊放，`settledCenterCardIsOnTop`／`centerCardStaysOnTopWhileSwiping`／`centerCardStaysOnTopWhileStepping`）；S5 牌組轉場測試 `Tests/UI/CoverFlowDeckTransitionSpikeTests.swift`（`advanceByOne`／`rebuild`／`firstDeckAfterEmptyMount`，預設關閉，`AZW_SPIKE_S5=1` 才跑）；D2 掛載結構（Q4，待做）；`LyricsFlowUITests`（Q3.5 起，待做） | ⬜ M8 目視 |
...
| H-10 | 純展示：無雙擊播放、無任何播控動詞（決策 6） | Plan 決策 6＋`docs/plans/2026-09-23-coverflow-queue-drawer.md` AC9 | **編譯期證據（強於任何運行時斷言）**：`MusicControlling` 協議七個方法（`playerState`／`currentTrack`／`nowPlaying`／`trackDetails`／`albumTracks`／`setLyrics`／`artworkData`，`Services/Music/MusicControlling.swift:90-100`）**無任何播控動詞**——Cover Flow 在編譯期就不可能觸發播放；`grep onTapGesture Features/CoverFlow/*.swift` 零命中。**AC9 三層證據（2026-09-23 補齊）**：①類型層＝上述協議簽名本身（無播控方法）；②執行期白名單＝`Tests/Services/MusicSelectorAllowListTests.swift`（`appProtocolDeclaresOnlyAllowedGetters`／`trackProtocolDeclaresOnlyAllowedGettersAndSetLyrics`／`negativeControlIsCaughtFourTimes`／`scriptingBridgeClassesCarryOnlyTheAllowedAppProtocols`／`processNotFoundMapsToNotRunning`）；③腳本閘門＝`Scripts/no_playback_gate.sh`＋`Tests/Services/NoPlaybackGateScriptTests.swift`（`repositoryPassesTheGate`／`forbiddenPatternIsCaught`／`allowedPatternsPass`）。**M8 明確不寫 live UITest**：原設想的「雙擊後 persistentID 不變」在 Music 播放時會因自動跳曲假失敗，且 test runner 讀 Music 需其自身的 TCC 授權；且該斷言在「無播控邏輯」與「有播控 bug」兩種情況下都通過，無區分力。**待做（Q4）**：「可點＝正在播∧位於正中的卡、只開 Editor」尚未實作（上述 `onTapGesture` 零命中即證明目前整個 Cover Flow 都不可點），列入該計劃 Q4 | ✅ M8＋AC9（2026-09-23 補齊三層） |
...
計劃 AC1–AC14 對應的新條目（H-12 起）於 Q6 收官時補齊。

...
| H-02 | 3D 形態五要素：中心正面／兩側 ±55° 斜角／負間距覆疊／倒影漸變／viewAligned 吸附 | Plan §4.8 | **幾何參數**已由 `CoverFlowGeometryTests`（15 條）釘住：旋轉 ±55° 含 clamp、縮放 1.0→0.82、負間距 −0.42×itemWidth、zIndex −\|d\|、perspective 0.55、首尾可達中心的 edgePadding。**觀感**（實際吸附行為、3D 疊放、resize）待 M8 並排目視——同 A-01／B-20 先例，依 Plan §1「視覺盡可能貼近（非像素級）」不做截圖比對。**U7 裁決（2026-09-23，`docs/plans/2026-09-23-coverflow-queue-drawer.md` §11）**：本分支上 `CoverFlowUITests`（原 H-02 閘門）退出回歸集合——資料模型已由整張專輯變為歷史／佇列，20 首 fixture 與探針前提不成立；檔案保留（Python 閘門測試 `Scripts/test_h02_gate_parsing.py` 仍讀它）但不再作本條證據。替代證據：`Tests/UI/CoverFlowStripStackingTests.swift`（缺陷 2 疊放，`settledCenterCardIsOnTop`／`centerCardStaysOnTopWhileSwiping`／`centerCardStaysOnTopWhileStepping`）；S5 牌組轉場測試 `Tests/UI/CoverFlowDeckTransitionSpikeTests.swift`（`advanceByOne`／`rebuild`／`firstDeckAfterEmptyMount`，預設關閉，`AZW_SPIKE_S5=1` 才跑）；D2 掛載結構（Q4 完成：外殼常駐、條帶只以空牌掛載一次，不再經過初始值路徑＝缺陷 3 的觸發條件不存在）；`LyricsFlowUITests`（使用者在場 #3 同一輪 8/8 綠；其 `waitForRaisedAndSettled` 要求播放卡按鈕出現＝條帶真的捲到正中） | ⬜ M8 目視 |
...
| H-24 | Music 未執行時任何路徑都不啟動它；執行中途 Music 退出 → AE 以錯誤返回（不重啟 Music） | AC13 | 代碼審查：`Services/Music/MusicAppleEventsClient.swift` `run()`（D10）只用 `NSRunningApplication.runningApplications(withBundleIdentifier:)` 取已執行實例、找不到即拋 `.notRunning`，未執行時不建立任何 `SBApplication`（程式內註解自陳：以 bundle ID 定址會觸發啟動，故改用 pid 定址）。單元：`MusicSelectorAllowListTests.processNotFoundMapsToNotRunning`（-600 錯誤映射為 `.notRunning`，不重啟） | ⬜ 中途退出 Music 待使用者在場手動驗（計劃 §13 暫停點清單） |
...
**總計 146 條**（含 8 條子條目 B-03a／B-11a／C-10a／D-01a／H-19a–d；2026-09-24 Cover Flow × 找歌詞新增 H-12–H-25 共 18 條後依 awk 實測：✅119／⚠️8／⬜17／🚫2）。

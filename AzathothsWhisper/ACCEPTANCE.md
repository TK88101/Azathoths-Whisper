...
| B-05 | 切歌且無詞→清空＋100ms 後自動 fetch；僅 Editor tab 激活時觸發（語義默認已向用戶聲明）。**Cover Flow × 找歌詞 D8 抑制規則（2026-09-23）**：已標記「沒有歌詞」的曲不自動抓詞；讀不到歌詞（unreadable）也不自動抓詞、不宣稱缺詞 | py:478-486＋`docs/plans/2026-09-23-coverflow-queue-drawer.md` D8 | `trackWithoutLyricsClearsAndAutoFetches`／`autoFetchSkippedWhenEditorTabInactive`／`AppModelTests.switchingTabTogglesEditorAutoFetchFlag`／`EditorViewModelTests.markedTrackDoesNotAutoFetch`（D8①：已標記不自動抓詞）／`EditorViewModelTests.unreadableLyricsNeitherAutoFetchNorClaimMissing`（unreadable 不自動抓詞、不宣稱缺詞） | ✅ M5＋Q3b |
| B-06 | Now Editing 卡片點擊＝手動 hydrate。**同一首歌重新讀取時不動畫面（計劃 AC6）** | py:745＋`docs/plans/2026-09-23-coverflow-queue-drawer.md` AC6 | `cardTapRequestsHydrate`（VM）＋`EditorView` 的 `onTapGesture` → `NowPlayingMonitor.forceRefresh`；`LyricsFlowReducerTests.sameTrackRefreshKeepsTheSurface`／`AppModelTests.sameTrackRefreshKeepsTheEditorTheUserOpened` | ✅ M5＋Q3b |
...
| E-04 | UI 內 30 個 i18n 位點三語正確呈現（原 M5 落檔時為 19／20 個，Cover Flow × 找歌詞新增 10 鍵） | py:874-882＋`docs/plans/2026-09-23-coverflow-queue-drawer.md` §9.5 | `LocalizationTests.everyUIKeyResolvesInAllThreeLanguages`（30 鍵 × 3 語，含 Cover Flow × 找歌詞 10 新鍵）＋`LocalizationTests.lyricsFlowKeysAreTranslated`（"新鍵三語"，10 組合逐字比對三語譯文）＋ja／zh-Hant 冷啟動 UI 斷言（導航／footer 實際文字） | ✅ M5＋Q3.5/Q4 |
...
| E-10 | 新鍵 `nav_coverflow` 三語（Cover Flow／封面瀏覽／カバーフロー）；v1 分頁移除後不再是分頁標題，改作 Cover Flow 把手標題（D12，撤回原 v1 改名） | Plan §4.6＋`docs/plans/2026-09-23-coverflow-queue-drawer.md` D12 | `LocalizationTests.coverFlowKeyIsTranslated`＋`ShellUITests.testColdStartInJapaneseLocalizesUIButNotMenus`／`.testColdStartInTraditionalChinese`（兩語冷啟動皆斷言把手標題可見，程式碼內明確標註「E-10」） | ✅ M5＋Q3.5/Q4 |
...
## H Cover Flow（29 條，對 spec＝Plan §4.8＋`docs/plans/2026-09-23-coverflow-queue-drawer.md` §4；其中 H-01／H-11 已隨 Cover Flow × 找歌詞 分支作廢，見下方 🚫）

| ID | 行為描述 | 依據 | 驗證方式 | 狀態 |
|---|---|---|---|---|
| H-01 | 第三 nav tab（Editor｜Batch｜Cover Flow），激活時懶加載 | Plan §4.8 | 🚫 已作廢：分頁已隨 Cover Flow × 找歌詞 Q3b 移除（`AppTab.coverFlow` 刪除），改由 Editor 內依歌詞狀態自動升降的一層取代（`docs/plans/2026-09-23-coverflow-queue-drawer.md` AC1／AC2）；原懶載入測試 `activatingTabWithEmptyListLoads`／`activatingTabWithLoadedListDoesNotReload` 已隨 Q3a 刪除（該計劃 §9.6） | 🚫 作廢（2026-09-23，Cover Flow × 找歌詞 Q3b） |
| H-02 | 3D 形態五要素：中心正面／兩側 ±55° 斜角／負間距覆疊／倒影漸變／viewAligned 吸附 | Plan §4.8 | **幾何參數**已由 `CoverFlowGeometryTests`（15 條）釘住：旋轉 ±55° 含 clamp、縮放 1.0→0.82、負間距 −0.42×itemWidth、zIndex −\|d\|、perspective 0.55、首尾可達中心的 edgePadding。**觀感**（實際吸附行為、3D 疊放、resize）待 M8 並排目視——同 A-01／B-20 先例，依 Plan §1「視覺盡可能貼近（非像素級）」不做截圖比對。**U7 裁決（2026-09-23，`docs/plans/2026-09-23-coverflow-queue-drawer.md` §11）**：本分支上 `CoverFlowUITests`（原 H-02 閘門）退出回歸集合——資料模型已由整張專輯變為歷史／佇列，20 首 fixture 與探針前提不成立；檔案保留（Python 閘門測試 `Scripts/test_h02_gate_parsing.py` 仍讀它）但不再作本條證據。替代證據：`Tests/UI/CoverFlowStripStackingTests.swift`（缺陷 2 疊放，`settledCenterCardIsOnTop`／`centerCardStaysOnTopWhileSwiping`／`centerCardStaysOnTopWhileStepping`）；S5 牌組轉場測試 `Tests/UI/CoverFlowDeckTransitionSpikeTests.swift`（`advanceByOne`／`rebuild`／`firstDeckAfterEmptyMount`，預設關閉，`AZW_SPIKE_S5=1` 才跑）；**2026-09-24 使用者實機**：換歌、自然播完後播放卡皆置中、未被鄰張蓋住（修正「兩側變動時停歪」後；`CoverFlowDeckSideChangeTests`）。D2 掛載結構（Q4 完成：外殼常駐、條帶只以空牌掛載一次，不再經過初始值路徑＝缺陷 3 的觸發條件不存在）；`LyricsFlowUITests`（使用者在場 #3 同一輪 8/8 綠；其 `waitForRaisedAndSettled` 要求播放卡按鈕出現＝條帶真的捲到正中） | ⬜ M8 目視 |
| H-03 | 中心下方標籤 "ARTIST // TITLE"（mono 排版語言） | Plan §4.8 | **文案組裝**＝`CoverFlowCenterLabelTests` 8 條（`composesArtistAndTitleWithDoubleSlash`／`returnsPlaceholderWhenCenterIDIsNilEvenWithCards`／`returnsPlaceholderWhenDeckIsEmpty`／`returnsPlaceholderWhenCenterIDNotInCards`／`returnsPlaceholderWhenDetailsAreNotLoadedYet`／`emptyArtistStillComposesRatherThanFallingBackToPlaceholder`／`cardsOfTheSameTrackShareTheLabel`／`slashesInsideFieldsArePreservedVerbatim`）——M8 把 `CoverFlowView.centerText` 提為純函數，現為 `CoverFlowCenterLabel.text(centerID:cards:details:)`（`Features/CoverFlow/CoverFlowCenterLabel.swift:13`）才可測。**不斷言大寫**：大寫來自 View 的 `.textCase(.uppercase)` 這個渲染修飾符，純函數與 `accessibilityLabel` 拿到的都是原串，對其斷言大寫會假失敗（M8 三角評審發現）。**mono 排版與大寫觀感**待 M8-14 目視 | ⬜ 觀感待 M8-14 目視（文案組裝已驗，見證據欄）——與 H-02／H-08 同口徑 |
| H-04 | 換歌後新的當前曲置中（左＝播過、右＝接下來）；≤3s（一個輪詢週期）自動平滑居中——**條件式保證**（2026-08-23 拍板，語意不變）：在正常 AE 回應與已定義的 timeout budget 下成立；若 Music.app 超過 AE budget 無回應，本輪允許延遲或缺少封面，但**不得**造成後續輪詢無界餓死。理由：`SBApplication` 的 AE 呼叫同步佔用串行佇列，Swift 層 timeout 無法撤回已送出的 Apple Event，client 端只能保證 bounded wait | Plan §4.8＋`docs/plans/2026-09-23-coverflow-queue-drawer.md` AC8／AC4 | **拆兩半，只關掉可關的那半**。**① 邏輯層「切歌後新曲置中」＝已驗**：`LyricsFlowModelTests.advancingWithinTheSameQueueMovesTheCentre`（換歌、檔案不重寫，中心移到清單下一首）／`CoverFlowViewModelDeckTests.advancingMovesTheCentreToTheNextCard`（同一份清單換牌與設中心同一次更新）。**② 調度層「不無界餓死」＝已驗**：`ArtworkAEFairnessTests.monitorCallWaitsBehindAtMostOneArtworkDuringPrefetch`（預取 10 張時 monitor 的 `currentTrack()` 前面至多一張 artwork）。**③ 1.5s 時間界＝未驗，待 M8-13 真機**：由 `SBApplication.timeout`（`MusicAppleEventsClient.swift:221` `artworkTimeoutTicks = 90`／`:246`）給出，**mock 無法覆蓋**——該測試檔開頭注解（`ArtworkAEFairnessTests.swift:7-13`）自己聲明「這裡不證明什麼：實際的 1.5 秒時間界…在此斷言秒數會是自欺」；且飢餓發生在 `MusicAppleEventsClient` 的 AE 串行化上，而 monitor 的 `currentTrack()` **根本不經過 `ArtworkService` 的佇列**（後者有獨立的 `pending`／`active`／`drainTask`）——**mock 在結構上不可達的是「時間界」本身，不是排程不變量**——後者可測，正因替身裡 monitor 與預取共用同一條 FIFO；而真實路徑上飢餓由 `SBApplication.timeout` 決定，替身沒有這個東西（M8 評審修正）。**故本條不得因引用 ①／② 而整條標 ✅** | ⬜ 時間界待 M8-13 真機（邏輯層／調度層已驗，見證據欄） |
| H-05 | 用戶手動滑走後不搶控制；下次真實切歌才拉回 | Plan §4.8 | 六條測試：`manualScrollSuppressesAutoCentreForTheSameTrack`／`sameTrackRefreshDoesNotClearTheOverride`（**真實切歌＝persistentID 變更**，forceRefresh 對同曲重發事件不算）／`realChangeClearsTheOverrideAndCentres`／`settledProgrammaticCallbackDoesNotSetTheOverride`（程式化捲動回呼不得誤判為使用者滑動）／`userCentreThatLeftTheDeckFallsBackToTheCurrentCard`（使用者居中的卡滑出牌組外後回退到當前曲，抑制解除）／`differentIDCallbackIsTreatedAsUserTakeover`。**降級理由（2026-09-11，Codex 裁決 #4；2026-09-23 核實名稱與行號）**：這六條只驗 VM 的邏輯層。`centerProgrammatically` 在**同一同步段**內 set→寫 centerID→clear `isCenteringProgrammatically`（`CoverFlowViewModel.swift:159-163`），而 ScrollView 的回寫發生在後續 frame——守衛在需要它的時刻已不存在；`differentIDCallbackIsTreatedAsUserTakeover` 的註解（`CoverFlowViewModelDeckTests.swift:106-107`）明文記載此限制：因 D6 不帶動畫、程式化居中不產生途經回呼，目前只有真正的拖曳會送來不同的 id。故「使用者滑走後不搶控制」的**交互整合**未驗，不得以邏輯層測試充當證據 | ⬜ 交互整合未驗（邏輯層已通過，見證據欄） |
| H-06 | 中心向兩側預取（原「切專輯→重建條目」半條已作廢，見驗證方式欄） | Plan §4.8 | **重建（🚫 已作廢，2026-09-23，Cover Flow × 找歌詞 Q3a）**：牌組不再由 `albumChanged` 事件消費，改由 Music 的「履歴」＋「接下來的歌」推導（`docs/plans/2026-09-23-coverflow-queue-drawer.md` AC8／AC8e）；原測試 `albumChangedQueriesUsingEventKeyNotCurrentTrack`／`staleAlbumLoadDoesNotOverwriteNewer`／`albumChangeResetsOverride` 已隨 Q3a 刪除（該計劃 §9.6）。**預取（保留，M7-P2；2026-09-23 換證據）**：視窗＝`CoverFlowPrefetchWindowTests` 7 條（`ordersByDistanceWithMovingSideFirst`／`excludesCenter`／`clampsAtBothEnds`／`emptyWhenSingleItem`／`emptyForInvalidCenterOrLimit`／`doesNotRepeatIDs`／`duplicatesAndTheCentresTrackAreExcluded`）；觸發＝`CoverFlowViewModelPrefetchTests` 的 `applyingPrefetchesAroundTheCentre`／`userScrollPrefetchesInTheScrollDirection`／`keyboardStepPrefetchesInTheStepDirection`／`aRebuiltDeckReplacesThePrefetchSet`／`hidingCancelsThePrefetch`／`hiddenModelDoesNotPrefetch`／`becomingVisiblePrefetchesTheCurrentWindow`／`stalePrefetchCommandsAreSkipped`／`duplicatePersistentIDsArePrefetchedOnce`；排程＝`ArtworkServiceTests` 的 `prefetchKeepsAEInFlightAtMostOne`（單槽不變量）／`explicitRequestIsServedBeforePendingPrefetch`（中心優先）／`explicitRequestJoinsPendingPrefetch`（同 ID 共享）／`prefetchReplacementDropsUnstartedEntries`（可取消）／`prefetchSkipsMemoryAndDiskHits`／`failedIDIsNotRefetchedUntilBackoffElapses`（退避）。**AE 公平性的邊界**：`ArtworkAEFairnessTests.monitorCallWaitsBehindAtMostOneArtworkDuringPrefetch` 只證明「送進 AE 的 artwork ≤ 1」這個排程不變量；「monitor 最多多等 1.5s」由該不變量＋`SBApplication.timeout` 推出，真機時序待 M8 | ✅ M7-P2（邏輯層，2026-09-23 換證據） |
...
| H-09 | 左右方向鍵步進（窗口內按鍵，無全局監聽） | Plan §4.8 | 已實作 `.focusable()` ＋ `.onKeyPress(.leftArrow/.rightArrow)`（`Features/CoverFlow/CoverFlowView.swift:19-24`，**窗口內**，無全域 hook——全域監聽會誤傷其他 app）；`stepMovesTheCentreByOffset`／`stepIsClampedAtBothEnds`（兩端不越界）／`keyboardStepSuppressesAutoCentre`（步進與拖曳同樣算使用者接管）／`stepOnAnEmptyDeckIsANoOp`；焦點行為待 UITests | ✅ M7-P1（邏輯層） |
| H-10 | 純展示：無雙擊播放、無任何播控動詞（決策 6） | Plan 決策 6＋`docs/plans/2026-09-23-coverflow-queue-drawer.md` AC9 | **編譯期證據（強於任何運行時斷言）**：`MusicControlling` 協議七個方法（`playerState`／`currentTrack`／`nowPlaying`／`trackDetails`／`albumTracks`／`setLyrics`／`artworkData`，`Services/Music/MusicControlling.swift:90-100`）**無任何播控動詞**——Cover Flow 在編譯期就不可能觸發播放；`Features/CoverFlow` 唯一的 `onTapGesture` 在條帶層（`CoverFlowView.swift:78`），只在播放卡位於幾何正中且點在封面正面時呼叫 `onTapPlayingCard`（開 Editor），不涉任何播放（見 H-14）。**AC9 三層證據（2026-09-23 補齊）**：①類型層＝上述協議簽名本身（無播控方法）；②執行期白名單＝`Tests/Services/MusicSelectorAllowListTests.swift`（`appProtocolDeclaresOnlyAllowedGetters`／`trackProtocolDeclaresOnlyAllowedGettersAndSetLyrics`／`negativeControlIsCaughtFourTimes`／`scriptingBridgeClassesCarryOnlyTheAllowedAppProtocols`／`processNotFoundMapsToNotRunning`）；③腳本閘門＝`Scripts/no_playback_gate.sh`＋`Tests/Services/NoPlaybackGateScriptTests.swift`（`repositoryPassesTheGate`／`forbiddenPatternIsCaught`／`allowedPatternsPass`）。**M8 明確不寫 live UITest**：原設想的「雙擊後 persistentID 不變」在 Music 播放時會因自動跳曲假失敗，且 test runner 讀 Music 需其自身的 TCC 授權；且該斷言在「無播控邏輯」與「有播控 bug」兩種情況下都通過，無區分力。**可點＝正在播∧位於正中的卡、只開 Editor**：Q4 實作、U#3 前預檢修正（卡片純展示不掛手勢，點擊與無障礙按鈕在條帶層），證據見 H-14 | ✅ M8＋AC9（2026-09-23 補齊三層） |
| H-11 | 穩定排序：disc→track number→AE 返回序 fallback（⚠️兼容修正，待簽字） | Plan §4.8 | 🚫 已作廢：卡片順序改為 Music 的「履歴」＋「接下來的歌」順序（`docs/plans/2026-09-23-coverflow-queue-drawer.md` AC8／AC8e），不再是整張專輯的 disc→track 排序；原測試 `itemsUseStableDisplayOrder` 已隨 Q3a 刪除（該計劃 §9.6）。`sortedForDisplay()`（`Services/Music/MusicControlling.swift:104`）本身仍在代碼中，但 Cover Flow 已不使用它排序 | 🚫 作廢（2026-09-23，排序改為播放順序；已告知使用者） |
| H-12 | 真實換歌到有詞曲（含已標記「無詞」／讀不到）→ 畫面升為 Cover Flow | `docs/plans/2026-09-23-coverflow-queue-drawer.md` AC1 | 單元：`LyricsFlowReducerTests.realChangeFollowsStatus`（present／markedNone／unknown 三種狀態皆判定→coverFlow）。整合：`AppModelTests.tickedTrackWithLyricsRaisesCoverFlow`。E2E：`LyricsFlowUITests.testPresentLyricsShowsCoverFlow` | ✅ |
| H-13 | 真實換歌到缺詞且未標記曲 → Cover Flow 降下、把手可見、Editor 自動抓詞（B-05） | AC2 | 單元：`LyricsFlowReducerTests.realChangeFollowsStatus`（missing→editor 分支）。整合：`AppModelTests.tickedMissingTrackShowsTheEditor`；自動抓詞沿用 B-05 既有證據 `EditorViewModelTests.trackWithoutLyricsClearsAndAutoFetches`。E2E：`LyricsFlowUITests.testMissingLyricsShowsEditor` | ✅ |
| H-14 | 點「正中且正在播」的那張 → Editor；點其他卡（含播放中但不在正中）→ 畫面、綁定曲、hydrate 計數皆不變；捲動跨中點途中沒有卡可點 | AC3 | 單元：`LyricsFlowReducerTests.tappingTheCentredPlayingCardOpensTheEditor`／`.tappingWhenNotCentredDoesNothing`／`.tappingWithNothingPlayingDoesNothing`。幾何單元：`LyricsFlowPresentationTests.centredWithinTolerance`（容差 ±0.2）／`.noCardIsCentredHalfwayBetweenTwo`（跨中點無卡可點）；`CoverFlowViewModelDeckTests.playingCardCountsAsCentredOnlyWhenTheStripSaysSo`。點擊許可與判定區（R10 抽出）：`CoverFlowGeometryTests.centreFaceIsTheSquareFrontOfTheCentredCard`／`.playingCardTapNeedsCentreAndFace`（含 CGRect 半開邊界）；無障礙標籤曲名：`CoverFlowViewModelDeckTests.playingCardTitleComesOnlyFromThePlayingCardsDetails`。逐幀取樣（沿用 `StackSession`）：`CoverFlowStripStackingTests.atMostOneCardIsCentredWhileSwiping`。E2E：`LyricsFlowUITests.testClickingPlayingCardOpensEditor`／`.testClickingOtherCardsDoesNothing`／`.testPlayingCardAwayFromTheCentreIsNotClickable`（使用者在場 #3，8/8 同一輪綠）；`.testOnlyTheFrontFaceOfThePlayingCardIsClickable`（無障礙按鈕四邊＝封面正面、點倒影不開、點正面角落開；R10 新增） | ✅（2026-09-24 使用者在場：`LyricsFlowUITests` 9/9） |
| H-15 | 寫入成功（trim 非空）→ 延遲 `riseDelay` 後升回；期間使用者操作（把手、點卡、標記、編輯文字）或真實換歌 → 取消升回；ABA 不被舊計時誤升回；空字串或失敗不升 | AC4 | 單元（`GatedPollClock`）：`LyricsFlowReducerTests.successfulWriteOfTheCurrentTrackSchedulesARise`／`.riseDueWithTheLiveTokenRaisesCoverFlow`／`.staleRiseAfterABADoesNothing`／`.userActionsCancelThePendingRise`／`.emptyWriteDoesNotRise`／`.emptyWriteAfterASuccessfulWriteCancelsThePendingRise`／`.writeOfANonCurrentTrackDoesNotMoveTheSurface`／`.failedWriteOnlyRefreshes`。升回延遲＝彩帶影格間隔×步數＝3.2s：`LyricsFlowPresentationTests.riseDelayIsTheConfettiLifetime`。整合：`AppModelTests.savingLyricsRaisesCoverFlowAfterTheConfetti`。E2E：`LyricsFlowUITests.testWritingLyricsRaisesCoverFlow` | ✅（2026-09-24 使用者實機：寫入後撒彩帶、約 3.2 秒升回，手感剛好；寫入誤報 FAILED TO SAVE 已修，見計劃 §13 收官） |
| H-16 | 「No lyrics for this song」只接受已知缺詞的當前曲（有詞／讀不到時整個動作不生效）；記標記、升回、取消自動抓詞；重啟後同曲不跳 Editor；日後寫入成功（非空）清除標記；空白寫入仍算缺詞 | AC5 | 單元：`LyricsFlowReducerTests.markingTheCurrentTrackRaisesAndCancelsAutoFetch`／`.markingIsIgnoredUnlessTheCurrentTrackIsMissing`（present／unknown 皆不生效）／`.markingANonCurrentTrackOnlyCancelsItsAutoFetch`；`NoLyricsMarksTests`（`markingPersistsAcrossStoreInstances`／`emptyPersistentIDIsRejected`／`aCorruptEntryDoesNotWipeTheOtherMarks`／`clearingRemovesOnlyThatTrack`／`markingTwiceKeepsASingleEntry`，注入臨時 defaults suite）。整合：`LyricsFlowModelTests.markingNoLyricsPersistsRaisesAndCancelsAutoFetch`／`.aMarkedTrackShowsCoverFlowNextTime`／`.successfulWriteClearsTheMark`／`.onlyAMissingCurrentTrackCanBeMarked`／`.emptyPersistentIDCannotBeMarked`；`AppModelTests.markingNoLyricsRaisesAndSurvivesRelaunch`。E2E：`LyricsFlowUITests.testMarkingNoLyricsPersistsAcrossRelaunch`（第一次啟動帶 reset env、重啟不帶） | ✅ |
| H-17 | 同曲重發（forceRefresh、notPlaying 後再播）只更新狀態與徽章，不動畫面、不取消待升回；例外①待升回期間讀回仍缺詞→取消升回並提示；例外②讀不到變缺詞且本場次未親手選過畫面→降下 | AC6 | 單元：`LyricsFlowReducerTests.sameTrackRefreshKeepsTheSurface`／`.sameTrackRefreshKeepsAPendingRise`／`.readBackMissingCancelsThePendingRise`／`.sameTrackMissingWithoutAPendingRiseKeepsTheSurface`／`.unknownThenMissingLowersTheEditor`／`.unknownThenMissingRespectsTheUsersOwnChoice`／`.anIneffectiveTapIsNotAChoice`／`.aRealChangeForgetsThePreviousChoice`／`.realChangeCancelsAPendingRise`。整合：`LyricsFlowModelTests.sameTrackRefreshKeepsTheUsersEditor`；`AppModelTests.sameTrackRefreshKeepsTheEditorTheUserOpened`／`.realChangeWhileTheSurfaceFlipsResetsTheUserOverride` | ✅ |
| H-18 | 徽章 ✓／✗／—／?（unknown）與 `LyricsStatus` 一致；優先序 present＞markedNone＞missing；歌詞讀取失敗＝unknown，不降下、不自動抓詞 | AC7 | 單元：`LyricsStatusTests.resolve`（`LyricsStatus.resolve` 全真值表 8 組，含「檔內有詞時標記無效」「純空白＝缺詞」「讀不到不折疊成缺詞」）；`LyricsFlowPresentationTests.badgeSymbol`。整合：`LyricsFlowModelTests.unreadableLyricsKeepCoverFlow`；`EditorViewModelTests.unreadableLyricsNeitherAutoFetchNorClaimMissing` | ✅ |
| H-19 | 佇列模式牌組（Queue.dat 前 K＋當前＋後 K，播放中置中）；同清單換歌以增量窗口更新（不重讀整份、不重置 Cover Flow）；清單內容變更依新清單重建；卡 ID 規則（`q:`/`o:`/`h:` 前綴、itID epoch、重複去重、兩側與中心同 ID 剔除）；序列種類判定（`shuffleMode`）；當前出現位置解析（相鄰項優先、唯一性、不相鄰時 pending） | AC8 | 單元（QueueSnapshot 解析，13 條）：`QueueSnapshotTests`，含 `orderedModeReadsListInFileOrderWithItemIDs`／`shuffledModeReadsShuffledListNotTheOriginalOrder`／`unclearShuffleStateIsUnusable`／`malformedShuffleModeIsUnusableEvenWithoutAShuffledList`／`noShuffleModeAndNoShuffledListReadsTheOnlySequence`／`anEntryWithoutTrackIDMakesTheSnapshotUnusable`／`multipleSegmentsAreTreatedAsUnusable`／`corruptDataIsUnusable`／`contentHashChangesWithOrderAndStaysStableForSameContent`／`realShapeLibraryShuffleSample`。單元（DeckSnapshot／卡 ID，11 條）：`DeckSnapshotTests`，含 `leftHistoryCentreCurrentRightUpcoming`／`theNextCardKeepsItsIDWhenItBecomesCurrent`／`theSameTrackMayAppearOnTheLeftAndInTheCentre`／`historyCardIDsCountOccurrencesFromTheNewest`／`cardIDsAreUniqueWithinADeck`／`theCentreCardSurvivesACollidingPlayedCard`。單元（佇列位置解析，計劃稱 OccurrenceResolver，實作為 `QueueSession`，18 條）：`QueueSessionTests`，含 `resolvesTheUniqueOccurrence`／`realChangeAdvancesToTheNextPosition`／`backSkipOntoADuplicateIsUnresolved`／`nonAdjacentJumpWaitsForTheNextPoll`／`ambiguousWithoutAHintIsUnresolved`／`rewriteKeepsThePositionThroughTheItemID`／`rebuildResolvesByUniquenessWhenTheOldItemIsGone`／`reusedItemIDForADifferentTrackBumpsTheEpoch`／`sameItemIDSameTrackKeepsTheEpochAcrossRewrites`／`duplicateItemIDsGetDistinctCardIDs`／`entriesWithoutItemIDUseOccurrenceWithinTheSnapshot`。正中不變、兩側變動仍停在正中：`CoverFlowDeckSideChangeTests.theCentreStaysWhenTheSidesChange`（5 種，2026-09-24 使用者實機回報後補）。整合：`LyricsFlowModelTests.queueFileGivesTheUpcomingCards`／`.advancingWithinTheSameQueueMovesTheCentre`；`CoverFlowViewModelDeckTests.advancingMovesTheCentreToTheNextCard` | ✅（U10 2026-09-24 唯讀實查：循序播放時兩處皆寫 `shuffleMode`＝off、只有 `list`；app 讀到右側 7 張） |
| H-19a | 左側＝`History.dat` 最近 K 筆（由舊到新排向中心）；新播完的歌出現在中心左側；檔案不可讀 → 左側改用本 app 觀察到的歷史 | AC8e | 單元：`HistorySnapshotTests`（6 條，含 `keepsTheMostRecentEntriesOldestFirst`／`entriesWithoutLibraryItemIDAreSkipped`／`persistentIDFromLibraryItemID`／`corruptDataIsUnusable`／`contentHashFollowsRecentEntries`／`realShapeHistorySample`）；`QueueFileSourceTests.readsHistoryKeepingTheRequestedTail`。整合：`LyricsFlowModelTests.historyFileGivesThePlayedCards`／`.withoutFilesTheLeftSideIsTheObservedHistory` | ✅ |
| H-19b | 右側退回模式（只管右側）：沒有已驗證有效快照、連續 2 次找不到當前曲、位置無法唯一解析、或 Music 未執行 → 右側空＋提示；單次讀取失敗或讀到寫入中的檔案保留最後有效快照，不切模式；條件恢復後自動回到佇列模式；歷史為有序集合（A→B→A 移到尾端不重複、當前曲不進、上限 50、空 persistentID 不進） | AC8b | 單元：`QueueFileSourceTests.unreadableFileIsAFailedReadNotAMissingFile`／`.readsOnceThenReportsUnchanged`／`.replacedFileIsReadAgain`／`.corruptFileReportsFailedAndRecoversOnReplacement`；`QueueSessionTests.failedReadWithoutAValidSnapshotIsUnavailable`／`.failedReadKeepsTheLastValidSnapshot`／`.currentNotInTheListIsPendingOnceThenUnavailable`／`.aSuccessfulResolveClearsTheMissStreak`／`.invalidationDropsTheSnapshotAndPosition`；`ListeningHistoryTests.replayMovesToTheEndWithoutDuplicating`（A→B→A）／`.emptyPersistentIDIsIgnored`／`.capacityDropsTheOldest`；`DeckSnapshotTests.withoutAResolvedQueuePositionTheCentreFallsBack`／`.notPlayingShowsOnlyTheLeftSide`。整合：`LyricsFlowModelTests.queueFileDisappearingFallsBackInsteadOfShowingAStaleQueue`（含同 mtime 內容恢復、原子替換） | ✅ |
| H-19c | Queue.dat 只讀：讀取碼只用 `Data(contentsOf:)`；該檔所在模組不得出現任何寫入／移動／刪除／建立檔案或設定屬性的 API；解析失敗不重試寫入、不拋到 UI | AC8c | 腳本閘門（AC9 ③ 增列 R4 規則，掃 `QueueFileSource／QueueSnapshot／HistorySnapshot` 三檔）：`Scripts/no_playback_gate.sh` ＋ `Scripts/test_no_playback_gate.py::NoPlaybackGateTests.test_repository_passes_the_gate`／`.test_each_forbidden_pattern_is_caught`／`.test_allowed_patterns_pass`。單元：`QueueFileSourceTests.unreadableFileIsAFailedReadNotAMissingFile`／`.corruptFileReportsFailedAndRecoversOnReplacement`（解析失敗回報 `.failed`，不拋出、不寫入） | ✅ |
| H-19d | 卡片詳情以 persistentID 批次讀取（去重後 ≤21 首一批）：單一背景 actor、最新者勝（快照版本 token，舊任務不得覆蓋新版）、總 deadline、快取（寫入／標記時更新該卡）；部分失敗 → 該卡狀態 unknown、只顯示封面；不阻塞事件分發、不餓死輪詢 | AC8d | 單元：`CardDetailsReaderTests`（6 條，含 `fetchesMissingDetailsInOneBatch`／`cachedDetailsAreNotRefetched`／`duplicateIDsAreRequestedOnce`／`updatingLyricsChangesTheCachedStatusSource`／`failureYieldsNothingAndIsNotCached`／`emptyIDsAreIgnored`）；`AEBudgetTests.eachEventGetsWhatIsLeftOfTheBudget`／`.lessThanOneTickLeftStopsTheBatch`／`.spendingSetsTheRemainingTimeoutOrRefuses`／`.detailsBudgetMatchesTheArtworkBound`；`AEBudgetReadColumnsTests.runningOutMidwayDropsTheWholeBatch`（總時限用完即整批放棄，Phase 3 /simcodex 第 1 輪修正）。公平性（`SerialAEQueueMusicClient`）：`ArtworkAEFairnessTests.monitorCallWaitsBehindAtMostOneArtworkDuringPrefetch`／`.serialQueueExecutesInFIFOOrder`。最新者勝：`CardDetailsReaderTests.aBatchArrivingLateDoesNotOverwriteANewerOne`／`.writtenLyricsSurviveABatchIssuedBeforeTheWrite`／`.aWriteBeforeTheFirstBatchLandsDiscardsThatBatch`（快取層）；`LyricsFlowModelTests.anOlderDetailsBatchArrivingLateIsDropped`（套用層世代號；變異：拿掉世代比對即紅）。欄位→詳情：`TrackDetailsColumnsTests.columnsBecomeDetailsInTrackOrder`／`.rowsWithoutAPersistentIDAreDropped`／`.incompleteBatchesAreRejected`。整合：`LyricsFlowModelTests.detailsAreFetchedForTheDeck` | ⬜ 1.5 秒總時限的真機時序待使用者試用時觀察（與 H-04③ 同口徑；預算邏輯、最新者勝、快取新舊皆已驗） |
| H-20 | 無播控三層：①類型層 `MusicControlling` 不含播控方法；②執行期以 `protocol_copyMethodDescriptionList` 列出 Music `@objc` 協議選擇器須 ⊆ 允許清單、`set*:` 只許 `setLyrics:`、required 方法＝0，負對照須回報違規；③腳本閘門掃全部 target 目錄禁 AppleScript／Accessibility／私有框架等管道 | AC9 | 類型層（編譯期，同 H-10 既有證據）：`Services/Music/MusicControlling.swift` 協議簽名無播控動詞。執行期：`MusicSelectorAllowListTests.appProtocolDeclaresOnlyAllowedGetters`／`.trackProtocolDeclaresOnlyAllowedGettersAndSetLyrics`／`.negativeControlIsCaughtFourTimes`／`.scriptingBridgeClassesCarryOnlyTheAllowedAppProtocols`／`.processNotFoundMapsToNotRunning`。腳本閘門：`Scripts/no_playback_gate.sh` ＋ `Scripts/test_no_playback_gate.py::NoPlaybackGateTests.test_repository_passes_the_gate`／`.test_each_forbidden_pattern_is_caught`／`.test_allowed_patterns_pass`（Swift 版 `Tests/Services/NoPlaybackGateScriptTests.swift` 已標 `.disabled`——app 宿主讀 `~/Documents` 會卡在系統存取授權，simcodex R1 移到 Python 版；**不可引為證據**） | ✅ |
| H-21 | 導覽區恰好兩個按鈕（Editor｜Batch，以 identifier 判定）；新字串三語齊 | AC10 | 單元：`LocalizationTests.everyUIKeyResolvesInAllThreeLanguages`（30 鍵 × 3 語）；`AppModelTests.tabsAreEditorAndBatchOnly`。E2E：`ShellUITests.testLaunchShowsEditorShellAfterSplash`（`app.buttons` 以 `nav-` identifier 前綴計數＝2，且 `COVER FLOW` 分頁不存在） | ✅ |
| H-22 | 每 Q 結束全量單元綠（skip 已知 flaky 一條）；`coverage_gate.sh` 判定 Services＋Infra ≥80%（豁免清單僅 AE client 一檔，理由／批准日／行數上限寫在腳本內）；新檔與改動過的 Features 檔逐檔 ≥80%（xccov 手順）；`LiveMusicTests` 在閘門外、寫入類測試需明示許可；UITests 回歸集合對分支點基線不新增紅燈；`test_h02_gate_*.py` 綠 | AC11 | 全量單元：`xcodebuild test`，收官時 618 條，只剩基線紅 `CoverFlowStripRenderGeometryTests`（2 條；斷言 8→7：條帶改 HStack 後初始值路徑停在 #0，正式 app 不走此路徑）。覆蓋率閘門：`Scripts/coverage_gate.sh`（JSON 報表模式＋豁免清單，僅 `Services/Music/MusicAppleEventsClient.swift`、行數上限 400）；`Scripts/test_coverage_gate.py::CoverageGateTests`（9 條，含 `test_passes_after_exemption_and_prints_both_ratios`／`test_fails_when_ratio_after_exemption_is_below_threshold`／`test_fails_when_exempted_file_is_missing_from_the_product_target`／`test_fails_when_exempted_file_appears_twice`／`test_fails_when_exemptions_exceed_the_line_budget`／`test_only_top_level_services_and_infra_count`）；實測豁免前 78.4%、豁免後 95.5% 通過（豁免檔 378／400 行）。無播控閘門：`Scripts/no_playback_gate.sh` ＋ `Scripts/test_no_playback_gate.py`。H-02 閘門：`python3 -m unittest discover -s Scripts -p 'test_h02_gate*.py'`（321 條通過）。UITests 回歸：`ShellUITests`＋`BatchUITests`（使用者在場 #3：新紅 0 條；C-21 兩次偶發單獨重跑皆綠；A-10 為 XCUITest 狀態回報卡住——進程監看證實點 Quit 後 ≤1.3s 即退出，與基線同簽名） | ✅（AC11 改寫 2026-09-24 使用者批准：View 宣告檔不以逐檔行覆蓋判定，逐檔 (a)–(d) 證據表見計劃 §13 收官；UITests 收官輪 LyricsFlow 9/9、Shell＋Batch 15/16，唯一紅＝A-10） |
| H-23 | 當前曲的曲目欄位與歌詞只來自同一個 `NowPlayingRead`（單一來源，同一次 `run` 對釘住的 track 讀出）；`NowPlayingRead` 是事件身分的唯一來源；替身回傳不完整讀取（歌詞讀取失敗）→ `unreadable` → 狀態 unknown | AC12 | 單元：`NowPlayingMonitorTests.trackAndLyricsComeFromASingleRead`／`.lyricsBelongToTheTrackServedInTheSameTick`／`.unreadableLyricsArriveAsNil`；`EditorViewModelTests.unreadableLyricsNeitherAutoFetchNorClaimMissing`。opt-in（真機）：`LiveMusicTests.nowPlayingReadsTrackAndLyricsTogether`／`.trackDetailsReadsTheCurrentTrackInOneBatch`（預設不入回歸集合，需 `AZW_MUSIC_TESTS=1`）。真機量測（非自動化測試，§13 S1）：釘住物件連讀 20 次屬性一致 | ✅ |
| H-24 | Music 未執行時任何路徑都不啟動它；執行中途 Music 退出 → AE 以錯誤返回（不重啟 Music） | AC13 | 代碼審查：`Services/Music/MusicAppleEventsClient.swift` `run()`（D10）只用 `NSRunningApplication.runningApplications(withBundleIdentifier:)` 取已執行實例、找不到即拋 `.notRunning`，未執行時不建立任何 `SBApplication`（程式內註解自陳：以 bundle ID 定址會觸發啟動，故改用 pid 定址）。單元：`MusicSelectorAllowListTests.processNotFoundMapsToNotRunning`（-600 錯誤映射為 `.notRunning`，不重啟） | ✅（2026-09-24 使用者在場：app 開著時 ⌘Q 關 Music，每 0.5 秒進程監看 90 秒未被叫回，app 保持上一首） |
| H-25 | 升起時方向鍵步進 Cover Flow、打字不進被遮住的 Editor；降下時 Cover Flow 不持有焦點 | AC14 | E2E：`LyricsFlowUITests.testKeyboardFocusFollowsSurface`。支援單元（既有 H-09 步進邏輯，D2 掛載結構下複用）：`CoverFlowViewModelDeckTests.stepMovesTheCentreByOffset`／`.stepIsClampedAtBothEnds` | ✅ |

計劃 AC1–AC14（含 AC8e／AC8b／AC8c／AC8d）對應條目：H-12–H-25（H-19a–H-19d 為 AC8 的四個子條目）。

**總計 146 條**（含 8 條子條目 B-03a／B-11a／C-10a／D-01a／H-19a–d；2026-09-24 Cover Flow × 找歌詞新增 H-12–H-25 共 18 條後依 awk 實測；收官後 ✅124／⚠️8／⬜12／🚫2）。

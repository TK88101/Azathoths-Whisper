...
| H-05 | 用戶手動滑走後不搶控制；下次真實切歌才拉回 | Plan §4.8 | 四條測試：`manualScrollSuppressesAutoCenter`／`sameTrackRefreshDoesNotClearOverride`（**真實切歌＝persistentID 變更**，forceRefresh 對同曲重發事件不算）／`realTrackChangeClearsOverrideAndCenters`／`programmaticCenteringDoesNotSetOverride`（程式化捲動回呼不得誤判為使用者滑動）。**降級理由（2026-09-11，Codex 裁決 #4）**：這四條只驗 VM 的邏輯層。`centerIfAllowed` 在**同一同步段**內 set→寫 centerID→clear `isCenteringProgrammatically`（`CoverFlowViewModel.swift:232-241`），而 ScrollView 的回寫發生在後續 frame——守衛在需要它的時刻已不存在；`CoverFlowViewModelTests.swift:260` 早已明文記載此限制。故「使用者滑走後不搶控制」的**交互整合**未驗，不得以邏輯層測試充當證據 | ⬜ 交互整合未驗（邏輯層已通過，見證據欄） |

...
/// 當前曲的身分：非空 persistentID，否則退回 `TrackInfo.signature`（與 monitor 換曲判據同源，N1）
...
enum LyricsFlowReducer {
    ...
    private static func nowPlaying(
        ...
        // 同曲重發（forceRefresh、停止後再播同曲）只更新狀態，不動畫面、不取消待升回（AC6）；
    ...
    private static func writeSucceeded(
        ...
        guard resultingStatus == .present, state.surface == .editor else { return (next, [.forceRefresh]) }
        next.riseSerial += 1
